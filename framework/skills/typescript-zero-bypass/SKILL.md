...
- No plumbing or persistence names such as `Join`, `Model`, `Type`, or `listOfAll...` when a behavior-oriented name exists
- No heterogeneous unions of unrelated models when a named domain input should exist instead
- No unreadable inline comparator callbacks that compress fallback logic and weak names into one expression
...
- Naming a function `listOfAllChecklistJoinCategory` instead of naming the behavior in domain terms
- Accepting `CategoryJoinChecklists | CategoryTypeModel | EconnectInformationModel['category']` in one parameter instead of naming the actual domain input
- Writing `sort((sortA, sortB) => (sortA.description || '').localeCompare(sortB.description || ''))` instead of making the comparison readable
...
- For sorting and filtering logic, extract a named comparator or normalize the compared fields before the callback becomes hard to read.
